<x-app-layout title="Edit Jam Mengajar">
    <div class="flex flex-col gap-7">
        <header>
            <h1 class="block text-2xl font-bold text-gray-800 sm:text-3xl dark:text-white">
                Edit Data Jam Mengajar
            </h1>
        </header>
        ...
        <div
            ...
            <form action="{{ route('admin.jam-mengajar.update', $data->jamMengajar->id) }}" class="flex flex-col gap-4"
                method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="nuptk"
                        ...
                        @foreach ($data->guru as $item)
                            <option value="{{ old('nuptk', $item->nuptk) }}"
                                {{ $item->nuptk == $data->jamMengajar->nuptk ? 'selected' : '' }}>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="flex rounded-md shadow-sm">
                        <input type="number" name="hour"
                            ...
                            placeholder="12" value="{{ old('hour', $data->jamMengajar->hour) }}" required>
                        <span
                                ...
                                <div class="relative flex items-start w-full">
                                    <div class="flex items-center h-5">
                                        <input id="days-{{ $i + 1 }}" name="days-{{ $i + 1 }}"
                                            ...
                                            {{ in_array($day, $data->jamMengajar->days) ? 'checked' : '' }}>
                                    </div>
                                    <label for="days-{{ $i + 1 }}"
